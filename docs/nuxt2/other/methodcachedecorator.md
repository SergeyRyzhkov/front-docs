...
2. если в кеше нет результата, то бдет выполнен метод, результат выполнения записан в кеш и возвращено значение
3. ключем в кеше является строка `наименование метода-аргументы`
